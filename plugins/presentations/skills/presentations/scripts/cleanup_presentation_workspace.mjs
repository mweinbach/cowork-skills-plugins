...
function assertSafeWorkspace(workspace) {
  ...
  const threadId = safePathSegment(process.env.COWORK_THREAD_ID);
  ...
  if (artifactSegment !== "presentations" || !isSafePathSegment(taskSegment)) {
    throw new Error(
      [
        ...
        "Expected a task workspace under outputs/<COWORK_THREAD_ID-or-manual-id>/presentations/<task-slug>.",
      ].join("\n"),
    );
  }

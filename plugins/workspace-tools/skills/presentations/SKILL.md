...
Use artifact-tool presentation JSX only. The bundled Cowork runtime provides
...
For generated raster images or reference comps, use the Cowork imagegen tool.
...
- `THREAD_ID=${COWORK_THREAD_ID:-manual-<timestamp-or-short-random-suffix>}`
...
is under `outputs/$THREAD_ID/presentations/`. If `COWORK_THREAD_ID` is

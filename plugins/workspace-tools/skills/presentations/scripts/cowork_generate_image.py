#!/usr/bin/env python3
"""
Write a Cowork imagegen prompt text file for a requested image.

This helper intentionally does not call any network API and does not read API
keys. The Presentations skill uses Cowork's imagegen tool for image creation;
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare a Cowork imagegen prompt text file from a text prompt."
    )
    parser.add_argument("prompt", help="Prompt to use with the Cowork imagegen tool.")
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "--reference-image",
        help="Optional reference image path to pass to Cowork imagegen.",
    )
    parser.add_argument(
        "--size",
...
def build_prompt_doc(args: argparse.Namespace, reference_image: Path | None) -> str:
    ...
    return "\n".join(
        [
            "# Cowork Imagegen Prompt",
            "",
            "Use the Cowork imagegen tool with this prompt. Do not call external image APIs from scripts.",
            "",
            "```json",
